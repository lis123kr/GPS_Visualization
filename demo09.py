...
def onpick(event):
	...
	if label == 'A' or label == 'G' or label == 'C' or label == 'T':
		...
		if label == 'ground' : bidx += 1
		else: vidx += 1
		if bidx == len(base_range): bidx = 0
		if vidx == len(variation_range): vidx = 0

		range_, pid, color = (str(base_range[bidx]), 0, 'black') if label == 'ground' else (str(variation_range[vidx]), 1, 'magenta')
...
if __name__ == '__main__':
	...
	spatches = [ mpatches.Patch(color=['black', 'magenta'][idx], label=['ground', 'variation'][idx]) for idx in range(2) ]
	leg_filter = Legend(fig, { 'spatches' : spatches, 'label' : ['ground_0.0', 'variation_5.0'], 'bbox':(0.40, .91, 0.24, .102), 'loc' : 3,
				'ncol' : 3, 'mode' : 'expand', 'borderaxespad' : 0.})	
